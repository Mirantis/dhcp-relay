...
func GetBPFSockFilterForDHCPv4Messages(mtu uint32) []unix.SockFilter {
	return []unix.SockFilter{
		...
		// If EtherType equals to IP (0x800), continue, else skip 15 instructions ahead.
		{Code: unix.BPF_JMP | unix.BPF_JEQ | unix.BPF_K, Jf: 15, K: unix.ETH_P_IP},
		// Load the IP header length field from the IP header.
		{Code: unix.BPF_LD | unix.BPF_B | unix.BPF_ABS, K: specs.EthernetHeaderSizeBits / 8},
		...
		// If IP header length is less than 5, skip 12 instructions ahead to drop the malformed packet.
		{Code: unix.BPF_JMP | unix.BPF_JGE | unix.BPF_K, Jt: 0, Jf: 12, K: specs.IPv4FieldIHLValueMinHeaderNoOptions},
		// If IP header length is greater than 5 (which means IP options are present), skip 11 instructions ahead.
		{Code: unix.BPF_JMP | unix.BPF_JGT | unix.BPF_K, Jt: 11, K: specs.IPv4FieldIHLValueMinHeaderNoOptions},
		// Load the Protocol field from the IP header.
		{Code: unix.BPF_LD | unix.BPF_B | unix.BPF_ABS, K: (specs.EthernetHeaderSizeBits + IPv4FieldOffsetProtocol) / 8},
		...
		{Code: unix.BPF_LD | unix.BPF_H | unix.BPF_ABS, K: (specs.EthernetHeaderSizeBits + IPv4FieldOffsetFlags) / 8},
		// If the packet is a fragment (fragment offset non-zero or More Fragments set), skip 7 instructions ahead.
		{Code: unix.BPF_JMP | unix.BPF_JSET | unix.BPF_K, Jt: 7, Jf: 0, K: 0x3FFF},
		// Load the length of the IP header into X.
		{Code: unix.BPF_LDX | unix.BPF_B | unix.BPF_MSH, K: specs.EthernetHeaderSizeBits / 8},
		...
		// Return the MTU as the snapshot length. This tells the kernel how many bytes to keep.
